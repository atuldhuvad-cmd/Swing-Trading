...
def test_valid_xlsx(client, db_session: Session):
...
def test_empty_xlsx(client, db_session: Session):
    wb = openpyxl.Workbook()
    out = io.BytesIO()
    wb.save(out)
    out.seek(0)
