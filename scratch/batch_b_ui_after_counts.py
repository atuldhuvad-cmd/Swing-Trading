"""Production counts after UI validation. Read-only."""
import sqlite3
...
print("max_eval", c.execute("SELECT MAX(evaluation_id) FROM candidate_evaluation_run").fetchone()[0])
print(
    "batch_a_ohlcv",
    c.execute(
        """
        ...
        WHERE s.nse_symbol IN ('ADANIENT','ADANIPORTS','APOLLOHOSP','ASIANPAINT','AXISBANK','BAJAJ-AUTO','BAJAJFINSV','BAJFINANCE','BEL','BHARTIARTL')
          AND d.series='EQ'
