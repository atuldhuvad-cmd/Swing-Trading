...
- Production baseline after the candidate refresh: SHA-256 `5ab98238badd90d59c0cb8626af6b55a62e3b73fb9043fa2ac0cfb25adeb3756`. It changed again with the ICICI import below; the runner guards in `scratch/` now use the newer value.
...
## ICICI recommendation import and scheduler (2026-09-23)

The sections above remain accurate for their runs.

- Commit `1255a22` added `--import` to `scratch/auto_download_broker_recs_icici.py` (default run still report-only). Disposable rehearsals on copies of production imported the four expected calls and, on a second run, nothing.
- Production import (HEAD `1255a22`, clean worktree, no app running): the live page returned 100 rows and matched the rehearsal exactly: four unambiguous `BUY` calls with name confidence at least 0.9, call dates 42-43 days old, and official ICICI report PDF links. The three calls already on file (APOLLOHOSP, HDFCBANK, NRBBEARING) were untouched.
- Backup before the import: `data\backups\swing_trading_pre_icici_import_20260923_141343.db` (SQLite backup API; all 27 tables identical to production, integrity `ok`, 0 FK violations).
- Result: broker recommendations 5 -> 9. Imported: CARYSIL, ASTRAMICRO, HINDALCO, GLAND, each `VERIFIED_PRIMARY` with `BROKER_WEBSITE` evidence and its report PDF link. Each stored row was compared independently with the collected page: rating, call date, entry, target, horizon and report link matched; stop loss and price-at-recommendation are NULL by design.
- Independent diff against the backup: exactly four rows added to each of `broker_recommendation`, `source_reference`, `recommendation_source` and `recommendation_status_history`; 0 existing rows changed and all 23 other tables unchanged (OHLCV 5419, fundamentals 20, candidate runs 70, criteria 630, risk/reward 53, trade journal 0). Integrity `ok`, 0 FK violations. 42 targeted tests passed and `GET /health` returned `{"status":"ok","foreign_keys":1}` on a disposable copy of the new production.
- Scheduler: `SwingTrading-ICICI-Recs`, Monday to Friday at 20:00, action `cmd.exe /d /c "D:\Swing Trading\scratch\run_broker_recs_icici.bat"` with working directory `D:\Swing Trading`, interactive user with no stored password, no overlapping runs, 30-minute execution limit. One manual run of the registered task exited 0 with "nothing new to record" and left all 27 tables unchanged.
- Production baseline: SHA-256 `bffa9e78df236d9cf47bdd9e08026cd6ea8769bd5d8d3da04e1660fe590f9049`. No candidate, OHLCV or fundamental data changed.
